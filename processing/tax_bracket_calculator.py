...
from config.config import TAX_BRACKETS_FILE


# calculate amount of tax or subsidy based on income brackets csv
def calculate_tax_bracket_amount(category: str, is_tax: bool, is_cumulative: bool, base_amount: float, base_salary: int) -> float:
    ...
    income_brackets = income_brackets[income_brackets["service"] == category]
    ...
    if is_cumulative:
        ...
        for row in income_brackets.itertuples(index=False):
            if base_salary > row.upper_bound:  # type: ignore
                cumulative_amount += (row.upper_bound - row.lower_bound) * row.rate  # type: ignore
            else:
                marginal_amount: float = (base_salary - row.lower_bound) * row.rate  # type: ignore
                break
        amount: float = cumulative_amount + marginal_amount

    # else, like hecs and medibank, it is a flat rate
    else:
        amount = base_amount * float((income_brackets.loc[bracket_row, "rate"]))  # type: ignore
